...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, img, sparse_input=None, mask=None,perceiver_input = None, loss_mask=None):
        ...
        def _match_channels(t, target_C):
            ...
            if target_C == 1 and t.size(1) > 1:
                return t.mean(dim=1, keepdim=True)  # or take [:, :1] if you prefer
        ...
        # ensure noise and predicted_noise have same channel count
        if predicted_noise.size(1) != noise.size(1):
            # choose the larger channel count as reference
        ...
        # also align masks to the same channel count if provided
        ...
        if loss_mask is not None:
            # assume loss_mask is shape (B, C, H, W), same as predicted_noise
            # Add a light penalty on the conditioned (mask) pixels
            lambda_cond = 0.05  # small weighting to prevent overfitting to conditioning
            combined_mask = loss_mask + lambda_cond * mask
            combined_mask = combined_mask.clamp(max=1.0)  # prevent values > 1
    ...
    def p_sample(self, xt, t, clip=True, sparse_input=None, perceiver_input = None, mask=None):
        ...
        batched_time = torch.full((xt.shape[0],), t, device=self.device, dtype=torch.long)
         ...
         # --- ALIGN CHANNELS FOR CONCAT (xt + sparse_input + mask) ---
        C = xt.size(1)  # this is self.unet.channel, e.g., 3
        if sparse_input is not None and sparse_input.size(1) != C:
            if sparse_input.size(1) == 1:
                sparse_input = sparse_input.repeat(1, C, 1, 1)
            else:
                sparse_input = sparse_input[:, :C, :, :]  # safe truncate
    
        if mask is not None and mask.size(1) != C:
            if mask.size(1) == 1:
                mask = mask.repeat(1, C, 1, 1)
            else:
                mask = mask[:, :C, :, :]  # safe truncate
        # ------------------------------------------------------------

        if sparse_input is not None and mask is not None:
            
            model_input = torch.cat([xt, sparse_input, mask], dim=1)  # shape (B, 9, H, W)
        else:
            model_input = xt  # fallback to vanilla DDPM
    ...
    def sample(self, batch_size=16, return_all_timestep=False, clip=True, min1to1=False,
        ...
        assert sparse_input is not None and mask is not None, "Must provide sparse_input and mask for conditioned sampling."
...
class DDIM_Sampler(nn.Module):
    ...
    @torch.inference_mode()
    def ddim_p_sample(self, model, xt, i, clip=True, sparse_input=None,perceiver_input=None, mask=None):
        """
        ...
        batched_time = torch.full((xt.shape[0],), t, device=self.device, dtype=torch.long)
         ...
         # --- ALIGN CHANNELS FOR CONCAT ---
        C = xt.size(1)
        if sparse_input is not None and sparse_input.size(1) != C:
            if sparse_input.size(1) == 1:
                sparse_input = sparse_input.repeat(1, C, 1, 1)
            else:
                sparse_input = sparse_input[:, :C, :, :]
        if mask is not None and mask.size(1) != C:
            if mask.size(1) == 1:
                mask = mask.repeat(1, C, 1, 1)
            else:
        ...
        if sparse_input is not None and mask is not None:
            ...
            model_input = xt
        pred_noise = model.unet(model_input, batched_time, perceiver_input)
    ...
    @torch.inference_mode()
    def sample(self, diffusion_model, batch_size, noise=None, return_all_timestep=False, clip=True, min1to1=False, sparse_input=None, perceiver_input = None, mask=None):
        """
        ...
        for i in tqdm(reversed(range(0, self.ddim_steps)), desc='DDIM Sampling', total=self.ddim_steps, leave=False):
            x_t_minus_1 = self.ddim_p_sample(diffusion_model, xt, i, clip, sparse_input=sparse_input, perceiver_input=perceiver_input, mask=mask)
            denoised_intermediates.append(x_t_minus_1)
            xt = x_t_minus_1
